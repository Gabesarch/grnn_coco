...
class LESCROART_MOC(Model):
    # torch.set_default_tensor_type('torch.cuda.FloatTensor')
    def initialize_model(self):
        print("------ INITIALIZING MODEL OBJECTS ------")
        self.model = LescroartMocModel()
...
class LescroartMocModel(nn.Module):
    def __init__(self):
        super(LescroartMocModel, self).__init__()
    ...
    def run_train(self, feed):
        ...
        if hyp.do_feat3d:
            ...
            self.summ_writer.summ_feat(f'feat3d/feat_halfmemX0', feat_halfmemX0, valid=valid_halfmemX0, pca=True)
            self.summ_writer.summ_feat(f'feat3d/feat_halfmemR', feat_halfmemR, valid=valid_halfmemR, pca=True)
            self.summ_writer.summ_oned(f'feat3d/valid_halfmemR', valid_halfmemR, bev=True, norm=False)
