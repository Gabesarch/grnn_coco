...
output_dir = '/projects/katefgroup/brain/encoding_model/grnn_feats_new_maxpool2_FIXED'
...
class COCO_GRNN(nn.Module):
    ...
    def run_extract(self):
        ...
        for images, _, file_ids in self.dataloader:
            ...
            with torch.no_grad():
                feat_memX, feat_loss = self.featnet(featXs_input, self.summ_writer, mask=occXs)
            ...
            if save_feats:
                # 3d pool
                feat_memX = self.maxpool3d(feat_memX)
                feat_memX = self.maxpool3d(feat_memX)
                feat_memX = self.maxpool3d(feat_memX)
                ...
                feats[idx] = feat_memX.detach().cpu().numpy() 
