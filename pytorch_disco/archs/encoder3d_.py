...
class Skipnet3d(nn.Module):
    def __init__(self, in_dim=1, mid_dim=64, out_dim=8):
        ...
        # self.layers = []
        self.down_in_channels = [in_dim, mid_dim, 2*mid_dim]#, 4*mid_dim]
        self.down_out_channels = [mid_dim, 2*mid_dim, 4*mid_dim, 8*mid_dim]
        self.down_ksizes = [4, 4, 4, 4]
        self.down_strides = [2, 2, 2, 2]
            
        padding = 1 #Note: this only holds for ksize=4 and stride=2!
        print('down dims: ', self.down_out_channels)

        for i, (in_channel, out_channel, ksize, stride) in enumerate(zip(self.down_in_channels, self.down_out_channels, self.down_ksizes, self.down_strides)):
            # print('3d CONV', end=' ')
             
            conv3d.append(nn.Sequential(
                nn.Conv3d(in_channels=in_channel, out_channels=out_channel, kernel_size=ksize, stride=stride, padding=padding),
                nn.LeakyReLU(),
        ...
        # self.up_in_channels = [8*chans, 8*chans]
        # self.up_out_channels = [4*chans, 4*chans]
        # self.up_bn_dims = [8*chans, 6*chans]
        # self.up_ksizes = [4, 4]
        # self.up_strides = [2, 2]
        
        self.up_in_channels = [4*mid_dim, 6*mid_dim]
        self.up_out_channels = [4*mid_dim, 4*mid_dim]
        self.up_bn_dims = [6*mid_dim, 5*mid_dim]
        self.up_ksizes = [4, 4]
        self.up_strides = [2, 2]
        
        padding = 1 #Note: this only holds for ksize=4 and stride=2!
        print('up dims: ', self.up_out_channels)

        for i, (in_channel, bn_dim, out_channel, ksize, stride) in enumerate(zip(self.up_in_channels, self.up_bn_dims, self.up_out_channels, self.up_ksizes, self.up_strides)):
             
            conv3d_transpose.append(nn.Sequential(
                nn.ConvTranspose3d(in_channels=in_channel, out_channels=out_channel, kernel_size=ksize, stride=stride, padding=padding),
                nn.LeakyReLU(),
            ))
