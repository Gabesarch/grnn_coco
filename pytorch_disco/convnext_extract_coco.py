...
# model_load = 'vicregl_convnext_small_alpha0p9'
# model_load = 'vicregl_convnext_small_alpha0p5'
# model_load = 'vicregl_convnext_small_alpha0p0'
model_load = 'vicregl_convnext_small_alpha0p1'
...
max_feature_size = 1000 # keep this many max components in pca
# layers = [
#     'out_initial',
#     'out_layer1',
#     'out_layer2',
...
general_model_name = 'convnext-small'
dataset = 'nsd'
##########%%%%%%%%%%%%%%%%%%%%%##################%%%%%%%%%%%%%%%%%%%#####################
...
class CONVNEXT(nn.Module):
    def __init__(self):
        ...
        vicreg_path = "/user_data/gsarch/checkpoints/vicregl/"

        pretrained = False
        if model_load=="vicregl_convnext_small_alpha0p9":
            model = torch.hub.load('facebookresearch/vicregl:main', 'convnext_small_alpha0p9')
        elif model_load=="vicregl_convnext_small_alpha0p1":
            model = torch.hub.load('facebookresearch/vicregl:main', 'convnext_small_alpha0p9')
            model_path = vicreg_path+"vicregl_alpha1.0.pth"
            checkpoint = torch.load(model_path)
            checkpoint = {k[1:]:v for k, v in checkpoint.items()}
            msg = model.load_state_dict(checkpoint)
            print(f"Loaded with message: {msg}")
        elif model_load=="vicregl_convnext_small_alpha0p75":
            # model = torch.hub.load('facebookresearch/vicregl:main', 'convnext_small_alpha0p75')
            model = torch.hub.load('facebookresearch/vicregl:main', 'convnext_small_alpha0p9')
            model_path = vicreg_path+"vicregl_alpha0.75.pth"
            checkpoint = torch.load(model_path)
            checkpoint = {k[1:]:v for k, v in checkpoint.items()}
            msg = model.load_state_dict(checkpoint)
            print(f"Loaded with message: {msg}")
        elif model_load=="vicregl_convnext_small_alpha0p5":
            model = torch.hub.load('facebookresearch/vicregl:main', 'convnext_small_alpha0p9')
            model_path = vicreg_path+"vicregl_alpha0.5.pth"
            checkpoint = torch.load(model_path)
            checkpoint = {k[1:]:v for k, v in checkpoint.items()}
            msg = model.load_state_dict(checkpoint)
            print(f"Loaded with message: {msg}")
        elif model_load=="vicregl_convnext_small_alpha0p25":
            model = torch.hub.load('facebookresearch/vicregl:main', 'convnext_small_alpha0p9')
            model_path = vicreg_path+"vicregl_alpha0.25.pth"
            checkpoint = torch.load(model_path)
            checkpoint = {k[1:]:v for k, v in checkpoint.items()}
            msg = model.load_state_dict(checkpoint)
            print(f"Loaded with message: {msg}")
        elif model_load=="vicregl_convnext_small_alpha0p0":
            model = torch.hub.load('facebookresearch/vicregl:main', 'convnext_small_alpha0p9')
            model_path = vicreg_path+"vicregl_alpha0.0.pth"
            checkpoint = torch.load(model_path)
            checkpoint = {k[1:]:v for k, v in checkpoint.items()}
            msg = model.load_state_dict(checkpoint)
            print(f"Loaded with message: {msg}")
        else:
            assert(False) # which model is this? 
    ...
    def forward(self, x):
        ...
        for i in range(4):
            if i==2:
                ...
                for i2 in range(len(self.stages[i])):
                    x = self.stages[i][i2](x)
                    if i2==13: # also take intermediate within stage 2
                        layers[f'out_{i}_{i2}'] = x
                layers[f'out_{i}_{i2}'] = x
            else:
                x = self.downsample_layers[i](x)
                x = self.stages[i](x)
...
class COCO_EXTRACT(nn.Module):
    def __init__(self):
        ...
        self.model = CONVNEXT() #torch.nn.Sequential(*(list(midas.pretrained.children())+list(midas.scratch.children()))[:-1])
        self.model.cuda()
        self.model.eval()
        print(self.model)
