...
class Vox_util(object):
    ...
    # def apply_pixX_T_memR_to_voxR(self, pix_T_camX, camX_T_camR, voxR, D, H, W):
    #     # mats are B x 4 x 4
    ...
    def unproject_rgb_to_mem_pc_single(self, rgb_camB, occ_memA, Z, Y, X, pixB_T_camA):
        ...
        xyz_pixB = utils_geom.apply_4x4(pixB_T_camA, xyz_camA) # 1 x N x 3
        normalizer = torch.unsqueeze(xyz_pixB[:,:,2], 2)
        EPS = 1e-6
        ...
        values = utils_samp.bilinear_sample_single(rgb_camB[0], x_valid_pixB[0], y_valid_pixB[0]).unsqueeze(0).permute(0,2,1)
        # 1 x Nv x C
