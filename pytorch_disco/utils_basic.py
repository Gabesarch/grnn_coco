...
if not multiprocess_flag.multi:
    import ipdb 
    st = ipdb.set_trace
EPS = 1e-6
...
def save_rgb(tensor,name,gt=False):
    if hyp.dataset_name == "carla":
        folder_name = "dump_carla"
    elif hyp.dataset_name == "replica":
        folder_name = "dump_replica"
    elif hyp.dataset_name == "clevr":
        folder_name = "dump_clevr"        
    else:
        folder_name = "dump_tp"
    for i,val in  enumerate(name):
        new_folder = join(folder_name,val)
        time_val = time.time()
        mkdir(new_folder)
        if gt:
            new_file = join(new_folder,str(time_val)+"_gt.png")
        else:
            new_file = join(new_folder,str(time_val)+"_pred.png")
        tensor_i = tensor[i]
        tensor_i = tensor_i.permute(1,2,0).detach().cpu().numpy()
        imsave(new_file,tensor_i)
